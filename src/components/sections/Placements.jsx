const PLACEMENT_STATS = [
  { label: "Highest Package", value: "₹42", sup: "L", sub: "Global product company · CSE '25" },
  { label: "Average Package", value: "₹8.4", sup: "L", sub: "Across all eligible branches" },
  ...
  return (
    <section id="placements" className="py-[100px] lg:py-[140px] px-5 lg:px-10 bg-bg relative">
      <div className="max-w-[1440px] mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-[100px] lg:items-end mb-20 pb-[50px] border-b border-border">
          <div>
          ...
          <div className="flex items-baseline gap-4">
            <div className="font-serif text-[90px] lg:text-[140px] font-light leading-[0.9] tracking-tighter text-accent italic">
              96<span className="text-[60px] text-text-dim italic">%</span>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 border-y border-border">
          {PLACEMENT_STATS.map((stat, i) => (
            <div key={i} className="p-10 lg:p-[40px_30px] border-r border-border last:border-r-0">
              <div className="font-mono text-[10px] tracking-[0.2em] uppercase opacity-40 mb-3.5 font-bold">{stat.label}</div>
              <div className="font-serif text-[56px] font-normal leading-none tracking-tight mb-2 text-text italic">
        ...
        <div className="mt-[90px]">
          <div className="flex justify-between items-center mb-10 pb-5 border-b border-border">
            ...
            <a href="#" className="font-mono text-[10px] tracking-[0.22em] uppercase text-accent font-bold hover:underline">View full list ↗</a>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-[1px] bg-border border border-border">
            {RECRUITERS.map((rec, i) => (
              <div key={i} className="bg-bg p-5 text-center flex items-center justify-center min-h-[72px] hover:bg-surface-light hover:text-text transition-colors font-serif text-[15px] font-medium tracking-tight text-text-dim cursor-default">
                {rec}
              </div>
            ))}
          </div>
