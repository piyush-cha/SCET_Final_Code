...
export default function Highlights() {
  return (
    <section className="bg-bg text-text py-[100px] lg:py-[120px] px-5 lg:px-10 relative overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto mb-[70px] flex flex-col lg:flex-row justify-between lg:items-end gap-10">
        ...
        <div className="flex gap-2">
          <button className="w-12 h-12 rounded-full border border-border flex items-center justify-center hover:bg-accent hover:border-accent hover:text-bg transition-all">
      ...
      <div className="max-w-[1440px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-[1px] bg-border border border-border">
        {HIGHLIGHTS.map((item, i) => (
          <div key={i} className={`p-[36px_30px] min-h-[380px] flex flex-col justify-between transition-colors ${item.feature ? "bg-accent text-bg" : "bg-bg hover:bg-surface"}`}>
            <div className="flex items-start justify-between mb-[30px]">
              <div className="font-serif text-[52px] font-normal leading-[0.9] tracking-tight italic">
